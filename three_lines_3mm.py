...
Nz = 100 #分割数
Nt = 300 #分割数
Δx = 0.1/Nx #0.1 mをNx分割
Δy = 0.1/Ny #0.1 mをNy分割
Δz = 0.1/Nz #0.1 mをNz分割
...
for k in range(Nx-1):
    for l in range(Ny-1):
        for n in range(Nz-1):
            if 21<=k<80 and l==50 and 49<=n<=50:
                Xe[k,l,n] = inn
            else:
                Xe[k,l,n] = out
# 入力電圧
@jit(nopython=True)
def gausian(t,σ,b):
    result = 2*(1/((2*3.14)**0.5)*σ)*math.exp(-((t-b)**2)/(2*σ**2))
    return result
        ...
        for k in range(Nx):
            for l in range(Ny-1):
                for n in range(Nz-1):
                    ...
                    elif (20<k<=80 and l==50 and n==48) or (20<k<=80 and l==50 and n==51) or (20<k<=80 and 49<=l<=51 and n==45):#導体
        ...
        for k in range(Nx-1):
            for l in range(Ny-1):
                for n in range(Nz):
                    ...
                    elif k==20 and l==50 and 45<n<=51:#導体
...
#NMとCMの導出
V＿n = np.zeros([Nt,Nx-1])
V＿s = np.zeros([Nt,Nx-1])
I_n = np.zeros([Nt,Nx])
I_s = np.zeros([Nt,Nx])

#コモンモードの計算
def common(V_n,V_s,I_n,I_s,U,Jx,Nt,Nx):
    for m in range(Nt):
        for k in range(Nx-1):
            V_n[m,k] = U[m,k,50,51]-U[m,k,50,48]
            V_s[m,k] = 0.5*(U[m,k,50,51]+U[m,k,50,48])-U[m,k,50,45]
        for k in range(Nx):
            I_n[m,k] = 0.5*(Jx[m,k,50,51]-Jx[m,k,50,48])
            I_s[m,k] = 0.5*(Jx[m,k,50,51]+Jx[m,k,50,48]-Jx[m,k,50,45])
    return V_n,V_s,I_n,I_s

V_n_result,V_s_result,I_n_result,I_s_result = common(V_n,V_s,I_n,I_s,U,Jx,Nt,Nx)
...
def anime(i):
    ...
    plt.plot(V_n_result[2*i,:],color = "blue",label="V_n")
    plt.plot(V_s_result[2*i,:],color="red",label="V_c")
    plt.legend()
    ...
    plt.plot(I_n_result[2*i,:],color="blue",label="I_n")
    plt.plot(I_s_result[2*i,:],color="red",label="I_c")
    plt.legend()
anim = animation.FuncAnimation(fig,anime,frames=149,interval=100)
# anim.save("three_lines_3mm_4.gif",writer="imagemagick")